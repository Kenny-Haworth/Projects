...
{
	private Node<T> root; //create a global root node
	...
	*/
	...
	/**
		Removes the specified entry from this priority queue.
		
		@param newEntry The entry to be removed
	*/
	public void remove(T newEntry)
	{
		if (isEmpty()) //the priority queue is empty, nothing can be removed
		{
			System.out.println("There is nothing to remove from this priority queue!");
		}
		else
		{	
			if (newEntry.compareTo(root.getData()) == 0) //the node to be removed is the root
			{
				root = root.getNext();
			}
			else //the node to be removed is not the root and is anywhere else
			{
				Node<T> temp = root;
