			...
			if (!cheese.equals("") && cheese.charAt(0) - '0' == input) //the section in the text file to print has been found
			{
				...
				while(inputData.hasNext()) //until the end of the text file has been reached
				{
					...
					if (!cheese.equals("") && cheese.charAt(0) == '	') //The line is stat changes or an enter. Any stat changes are always at the end of a section
					{
						...
						if (cheese.charAt(cheese.length()-1) == ';') //the stat change is the last line in the section, remove the ";"
						{
							statEnding = true;
						}
					...
					if (!cheese.equals("") && cheese.charAt(cheese.length()-1) == ';') //the end of the section has been reached
					{
						complete = true;
						
						StringTokenizer token = new StringTokenizer(cheese, ";"); //remove the ; at the end of the statement
						
						if (statEnding) //the last line is a stat and needs a different delay
						{
							print(token.nextToken() + "\n", resourceGainDelay);
							enter(); //for the last line the user must press enter
							break;
						}
						else //the last line is not a stat
						{
