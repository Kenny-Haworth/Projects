...
def train(model, image, actionSize):
	...
	#chosen combined action = np.argmax[q[0]]
	#chosen action = lookup_table[combined_action]
...
if __name__ == "__main__":
	env = retro.make(game='SuperMarioBros-Nes', state='Level3-1', record = './World3')
	...
	for episode in range(numberOfEpisodes):
		...
		if (newBest == False) and (episode != 0): #delete the replay, it wasn't good enough to save
			...
			os.remove("./World3/SuperMarioBros-Nes-Level3-1-" + string + ".bk2")
