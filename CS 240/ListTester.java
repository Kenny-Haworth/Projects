...
public class ListTester
{
	...
	{
		DLNodeList<String> list = new DLNodeList<String>();
		boolean loop = true;
		...
		while (loop)
		{
			...
			if (input.equalsIgnoreCase("ADD"))
			{
				...
				input = kb.nextLine();
				list.add(input);
				System.out.println(input + " has been added to the bottom of this list.");
			}
			else if (input.equalsIgnoreCase("ADD2"))
			{
				System.out.println("Enter the number position you would like to add: ");
				intInput = kb.nextInt();
				kb.nextLine();
				System.out.println("Enter the item you would like to add to the list: ");
				secondInput = kb.nextLine();
				list.add(intInput, secondInput);
				System.out.println(secondInput + " has been added to position " + intInput + " on the list.");
			}
